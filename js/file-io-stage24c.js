...
{{PREVIOUS_OUTPUT}}`;
  ...
  async function maybeReviewImportedDeckWithAi(importedSlides, deckTitle){
    ...
    function applyImportedSlides(importedSlides, opts={}){
      const incoming = stage43ahSanitizeImageBlobBatch((importedSlides || []).map(stage43gPrepareImportedSlide).filter(Boolean));
      ...
      try{ const previewEl = doc().getElementById('preview'); if(previewEl) previewEl.innerHTML = ''; }catch(_err){}
    ...
    async function importSelectedFiles(fileList){
      ...
      if(usedExtractionBackend){
        ...
        imported = stage43ahSanitizeImageBlobBatch(imported);
        const skipBackgroundAiRepair = isDocAiSemanticImportedBatch(imported) || stage43ahIsImageBlobBatch(imported);
