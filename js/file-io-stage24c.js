/* Stage 41V file/import workflow helpers: AI preserve-and-merge import keeps source math/figures when AI cleanup drops them.
...
{{PREVIOUS_OUTPUT}}`;
    ...
    function mergeSourcePreservationIntoAiDeck(deck, sourceSlides, originalProblems){
      ...
      try{ global.__LUMINA_STAGE41V_LAST_PRESERVE_MERGE = stats; }catch(_err){}
      return deck;
    }
    ...
    let text = await requestAiReviewText(endpoint, token, provider, model, system, input, 0.15, 42000);
      ...
      if(problems.length){
        ...
        problems = findAiImportDeckProblems(deck, "").concat(findAiImportMissingSourceProblems(deck, slides));
        if(problems.length){
          showToast('AI still dropped source math/figures; merging recovered source blocks…');
          deck = mergeSourcePreservationIntoAiDeck(deck, slides, problems);
          problems = findAiImportDeckProblems(deck, "").concat(findAiImportMissingSourceProblems(deck, slides));
          if(problems.length){
            throw new Error('AI deck still failed cleanup validation after preserve-merge: ' + problems.join('; '));
          }
        }
      }
      try{ global.__LUMINA_STAGE41V_LAST_AI_IMPORT_REVIEW = { ok:true, endpoint, provider, model, inputSlides:(slides||[]).length, outputSlides:deck.slides.length, validation:'passed-with-preserve-merge', preserveMerge:global.__LUMINA_STAGE41V_LAST_PRESERVE_MERGE || null, at:new Date().toISOString() }; global.__LUMINA_STAGE41R_LAST_AI_IMPORT_REVIEW = global.__LUMINA_STAGE41V_LAST_AI_IMPORT_REVIEW; }catch(_err){}
      return deck;
    }
    async function maybeReviewImportedDeckWithAi(importedSlides, deckTitle){
      ...
      try{
        ...
        // Stage 41V: do not leave the user with no slides when the AI preserve-merge path
