/*
 * copilot.js
 * Stage 6 migration: ChatGPT/OpenAI Copilot workflow extracted from legacy-app.js.
 *
 * This remains a classic browser script, not an ES module. legacy-app.js injects
 * the app state and rendering hooks through createApi() so the stable startup
 * path remains unchanged while migration continues.
 */
(function(global){
  'use strict';
  ...
  function required(deps, name){
    if(!deps || !(name in deps)){
      throw new Error('LuminaCopilot missing dependency: ' + name);
    }
    return deps[name];
  }
  ...
  function createApi(deps){
    ...
    const currentDeckData = required(deps, 'currentDeckData');
    const normalizeSlide = required(deps, 'normalizeSlide');
    const normalizeBlock = required(deps, 'normalizeBlock');
    const applySlideToForm = required(deps, 'applySlideToForm');
    const buildPreview = required(deps, 'buildPreview');
    const renderDeckList = required(deps, 'renderDeckList');
    ...
    const setSlides = required(deps, 'setSlides');
    const getActiveIndex = required(deps, 'getActiveIndex');
    const setActiveIndex = required(deps, 'setActiveIndex');

    const copilotEls = {
      prompt: doc.getElementById('copilotPrompt'),
      slideCount: doc.getElementById('copilotSlideCount'),
      tone: doc.getElementById('copilotTone'),
      mode: doc.getElementById('copilotMode'),
      model: doc.getElementById('copilotModel'),
      endpoint: doc.getElementById('copilotEndpoint'),
      apiKey: doc.getElementById('copilotApiKey'),
      status: doc.getElementById('copilotStatus'),
      resultJson: doc.getElementById('copilotResultJson')
    };
    ...
    function loadCopilotSettings(){
      try{
        const raw = global.localStorage.getItem(COPILOT_SETTINGS_STORAGE);
        ...
        const key = global.localStorage.getItem(COPILOT_API_KEY_STORAGE);
    ...
    function saveCopilotSettings(saveKey=false){
      try{
        ...
        global.localStorage.setItem(COPILOT_SETTINGS_STORAGE, JSON.stringify(s));
        if(saveKey && copilotEls.apiKey){
          const key = copilotEls.apiKey.value.trim();
          if(key) global.localStorage.setItem(COPILOT_API_KEY_STORAGE, key);
          else global.localStorage.removeItem(COPILOT_API_KEY_STORAGE);
    ...
    async function callCopilot(kind){
      ...
      const res = await global.fetch(endpoint, { method:'POST', headers, body:JSON.stringify(body) });
    ...
    function appendCopilotSlides(deck){
      ...
      const nextSlides = (existing.length ? existing : []).concat(newSlides);
      setSlides(nextSlides);
      setActiveIndex(nextSlides.length - newSlides.length);
      applySlideToForm(nextSlides[getActiveIndex()]);
      buildPreview();
      renderDeckList();
    ...
    async function generateCopilotSlide(applyMode){
      try{
        ...
        global.alert(err.message || 'Copilot failed.');
      }
    }
    ...
    function initCopilot(){
      loadCopilotSettings();
      doc.getElementById('saveCopilotKeyBtn')?.addEventListener('click', ()=>saveCopilotSettings(true));
      ...
      doc.getElementById('copilotDraftSlideBtn')?.addEventListener('click', ()=>generateCopilotSlide('replace'));
      doc.getElementById('copilotAddSlideBtn')?.addEventListener('click', ()=>generateCopilotSlide('append'));
      doc.getElementById('copilotGenerateDeckBtn')?.addEventListener('click', generateCopilotDeck);
      doc.getElementById('copilotApplyFirstSlideBtn')?.addEventListener('click', ()=>{ try{ applyCopilotFirstSlide(); }catch(err){ global.alert(err.message || 'Could not apply Copilot slide.'); } });
      doc.getElementById('copilotAppendResultBtn')?.addEventListener('click', ()=>{ try{ appendCopilotSlides(); }catch(err){ global.alert(err.message || 'Could not append Copilot slides.'); } });
      doc.getElementById('copilotReplaceDeckBtn')?.addEventListener('click', ()=>{ try{ replaceDeckWithCopilot(); }catch(err){ global.alert(err.message || 'Could not replace deck.'); } });
    }
  ...
  global.LuminaCopilot = { createApi };
})(window);
