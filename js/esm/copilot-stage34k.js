/* Stage 36X: Copilot core with deck-only dev prompt, reference-link web search, and stronger figure directives.
...
function createApi(deps){
  ...
  const runtimeStatus = deps.copilotRuntimeStatus || { stage:'stage36x-20260427-1' };
  ...
  updateCopilotRuntime({ stage:'stage36x-20260427-1', lastRuntimeLoadedAt:new Date().toISOString(), devPromptFile:COPILOT_DEV_PROMPT_FILE, deckPromptFile:COPILOT_DECK_PROMPT_FILE, deckPromptAppliesTo:'deck-only' });
  ...
  function cacheBustedPromptUrl(fileName){
    ...
    const version = safeString(deps.stage || runtimeStatus.stage || 'stage36x-20260427-1');
