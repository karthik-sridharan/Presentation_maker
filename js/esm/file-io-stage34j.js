...
{{PREVIOUS_OUTPUT}}`;
    ...
    async function loadEditableAiPrompt(path, fallback){
      ...
      try{
        ...
        const url = editablePromptUrl(key + sep + 'stage=stage42y-image-patch-real-image-blocks-20260512-1&promptCacheBust=' + Date.now());
  ...
  function mergeSimpleAiRepairWithSource(deck, sourceSlides){
    ...
    const stats = { stage:'stage42y-image-patch-real-image-blocks-20260512-1', sourceSlides:sourceSlides.length, outputSlides:deck.slides.length, imageAssetsRestored:0, layoutsPreserved:0, blocksRestored:0, slidesRestored:0, mathFieldsRepaired:0, at:new Date().toISOString() };
    const outputSlides = [];
...
function mergeSourcePreservationIntoAiDeck(deck, sourceSlides, originalProblems){
  ...
  function stage42sPublishImportStatus(update){
    try{
      ...
      var next = Object.assign({}, prev, update || {}, { stage:'stage42y-image-patch-real-image-blocks-20260512-1', updatedAt:new Date().toISOString() });
  ...
  try {
    ...
    global.__LUMINA_STAGE41V_FILE_IO_READY = { stage:'stage42y-image-patch-real-image-blocks-20260512-1', ready:true, at:new Date().toISOString(), apiKeys:Object.keys(api) };
