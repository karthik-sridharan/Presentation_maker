...
{{PREVIOUS_OUTPUT}}`;
    ...
    async function loadEditableAiPrompt(path, fallback){
      ...
      try{
        ...
        const url = editablePromptUrl(key + sep + 'stage=stage43m-mathpix-selected-image-detection-20260513-1&promptCacheBust=' + Date.now());
  ...
  function mergeSimpleAiRepairWithSource(deck, sourceSlides){
    ...
    const stats = { stage:'stage43m-mathpix-selected-image-detection-20260513-1', sourceSlides:sourceSlides.length, outputSlides:deck.slides.length, imageAssetsRestored:0, layoutsPreserved:0, blocksRestored:0, slidesRestored:0, mathFieldsRepaired:0, at:new Date().toISOString() };
    const outputSlides = [];
...
function parseAiPatchOrDeckResponseText(text, fallbackTitle, sourceSlides){
  ...
  async function maybeReviewImportedDeckWithAi(importedSlides, deckTitle){
    ...
    try{
      ...
      const stats = deck && deck.aiPatchStats || globalThis.__LUMINA_STAGE43A_MATHPIX_REPAIR || null;
  ...
  function stage42sPublishImportStatus(update){
    try{
      ...
      var next = Object.assign({}, prev, update || {}, { stage:'stage43m-mathpix-selected-image-detection-20260513-1', updatedAt:new Date().toISOString() });
  ...
  function extractPresentationFile(file) {
    ...
    if (token) headers.Authorization = 'Bearer ' + token;
    var startedAt = Date.now();
    stage42sPublishImportStatus({ phase:'waiting-for-backend', message:'Uploaded file; waiting for extraction backend… ' + (engine === 'docai' ? 'Document AI fast mode is on: backend AI rebuild is skipped, but rendered review choices are requested.' : (engine.indexOf('all-image') >= 0 ? 'PyMuPDF image-patch mode is on: every text or math region is kept as a real cropped image block for later extraction.' : (engine.indexOf('math-image') >= 0 ? 'PyMuPDF math-image hybrid is on: prose stays editable and math-like regions are cropped as image blobs.' : 'Extraction is running.'))), endpoint:stage42sCompactEndpoint(endpoint), extractEngine:engine, filename:file && file.name || '', fileSize:file && file.size || 0, pending:true, startedAt:new Date().toISOString() });
  ...
  try {
    ...
    global.__LUMINA_STAGE41V_FILE_IO_READY = { stage:'stage43m-mathpix-selected-image-detection-20260513-1', ready:true, at:new Date().toISOString(), apiKeys:Object.keys(api) };
