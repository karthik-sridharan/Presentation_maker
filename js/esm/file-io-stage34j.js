...
function parseAiPatchOrDeckResponseText(text, fallbackTitle, sourceSlides){
  ...
  function applyImportedSlides(importedSlides, opts) {
    ...
    try {
      var preImportPreviewEl = doc().getElementById('preview');
      if (preImportPreviewEl) preImportPreviewEl.innerHTML = '';
      globalThis.__LUMINA_STAGE43AB_PREIMPORT_PREVIEW_CLEARED = { ok:true, mode:mode, incoming:incoming.length, at:new Date().toISOString() };
    } catch (_err) {}
    if (mode !== 'replace') {
