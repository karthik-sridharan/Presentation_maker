...
export function createApi(deps) {
  ...
  function extractionEngineValue() {
    ...
    if(['pymupdf','pymupdf-math-image','pymupdf-all-image','mineru-pymupdf','marker','hybrid','docai'].indexOf(value) < 0) value = 'hybrid';
...
{{PREVIOUS_OUTPUT}}`;
    ...
    async function loadEditableAiPrompt(path, fallback){
      ...
      try{
        ...
        const url = editablePromptUrl(key + sep + 'stage=stage43s-block-action-backend-endpoint-fix-20260514-1&promptCacheBust=' + Date.now());
  ...
  function mergeSimpleAiRepairWithSource(deck, sourceSlides){
    ...
    const stats = { stage:'stage43s-block-action-backend-endpoint-fix-20260514-1', sourceSlides:sourceSlides.length, outputSlides:deck.slides.length, imageAssetsRestored:0, layoutsPreserved:0, blocksRestored:0, slidesRestored:0, mathFieldsRepaired:0, at:new Date().toISOString() };
    const outputSlides = [];
...
function parseAiPatchOrDeckResponseText(text, fallbackTitle, sourceSlides){
  ...
  function stage42sPublishImportStatus(update){
    try{
      ...
      var next = Object.assign({}, prev, update || {}, { stage:'stage43s-block-action-backend-endpoint-fix-20260514-1', updatedAt:new Date().toISOString() });
  ...
  try {
    ...
    global.__LUMINA_STAGE41V_FILE_IO_READY = { stage:'stage43s-block-action-backend-endpoint-fix-20260514-1', ready:true, at:new Date().toISOString(), apiKeys:Object.keys(api) };
