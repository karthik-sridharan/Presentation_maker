...
export function createApi(deps) {
  ...
  function extractPresentationFile(file) {
    ...
    return fetch(endpoint, { method:'POST', headers:headers, body:form }).then(function (res) {
      return res.text().then(function (text) {
        ...
        try { globalThis.__LUMINA_STAGE41J_LAST_EXTRACTION = { ok:true, slideCount:payload.slides.length, source:payload.source || null, meta:payload.meta || null, warnings:payload.warnings || [], endpoint:endpoint, filename:file && file.name || '' }; } catch (_err) {}
        return payload;
      });
    });
  }
  function applyImportedSlides(importedSlides, opts) {
    ...
    try { globalThis.__LUMINA_STAGE41J_LAST_IMPORT = { requestedSlides:(importedSlides||[]).length, normalizedSlides:incoming.length, mode:opts && opts.mode || 'append', at:new Date().toISOString() }; } catch (_err) {}
