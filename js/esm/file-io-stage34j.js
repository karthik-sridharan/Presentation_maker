/* Stage 42L: browser-compatible ES module file/import workflow helpers with patch-task AI repair.
...
{{PREVIOUS_OUTPUT}}`;
    ...
    async function loadEditableAiPrompt(path, fallback){
      ...
      try{
        ...
        const url = editablePromptUrl(key + sep + 'stage=stage42l-import-flow-restore-ai-stale-fix-20260511-1&promptCacheBust=' + Date.now());
  ...
  function mergeSimpleAiRepairWithSource(deck, sourceSlides){
    ...
    const stats = { stage:'stage42l-import-flow-restore-ai-stale-fix-20260511-1', sourceSlides:sourceSlides.length, outputSlides:deck.slides.length, imageAssetsRestored:0, layoutsPreserved:0, blocksRestored:0, slidesRestored:0, mathFieldsRepaired:0, at:new Date().toISOString() };
    const outputSlides = [];
...
function mergeSourcePreservationIntoAiDeck(deck, sourceSlides, originalProblems){
  ...
  function applyAiRepairPatchesToSource(deckTitle, sourceSlides, patchResult){
    ...
    try{
      globalThis.__LUMINA_STAGE42L_PATCH_AI_IMPORT_REPAIR = stats; globalThis.__LUMINA_STAGE42J_PATCH_AI_IMPORT_REPAIR = stats; globalThis.__LUMINA_STAGE42I_PATCH_AI_IMPORT_REPAIR = stats; globalThis.__LUMINA_STAGE42H_PATCH_AI_IMPORT_REPAIR = stats;
  ...
  async function callImportAiReview(deckTitle, slides){
    ...
    const stats = deck && deck.aiPatchStats || globalThis.__LUMINA_STAGE42L_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42J_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42I_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42H_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42C_SIMPLE_AI_IMPORT_REPAIR || null;
    try{
  ...
  async function maybeReviewImportedDeckWithAi(importedSlides, deckTitle){
    ...
    try{
      ...
      const stats = deck && deck.aiPatchStats || globalThis.__LUMINA_STAGE42L_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42J_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42I_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42H_PATCH_AI_IMPORT_REPAIR || null;
      ...
      // Stage 42F: do not leave the user with no slides when the AI review path
  ...
  function applyStage42eBackgroundAiRepair(rawBatchSlides, repairedDeck, startIndex, batchId, deckTitle){
    ...
    const patchStats = repairedDeck && repairedDeck.aiPatchStats || globalThis.__LUMINA_STAGE42L_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42J_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42I_PATCH_AI_IMPORT_REPAIR || globalThis.__LUMINA_STAGE42H_PATCH_AI_IMPORT_REPAIR || null;
  ...
  function importSelectedFiles(fileList) {
    ...
    return chain.then(function () {
      ...
      return reviewExtractedSlidesWithAlternates(imported, deckTitle).then(function(reviewedSlides){
        ...
        return { deckTitle:deckTitle, slides:reviewedSlides, theme:null, presentationOptions:null, aiReviewed:false, aiRepairPending:aiReviewAfterImportEnabled() };
      });
    }).then(function (importDeck) {
      imported = importDeck.slides || imported;
      deckTitle = importDeck.deckTitle || deckTitle;
      var mode = importModeValue();
      var startIndex = mode === 'replace' ? 0 : getSlides().length;
      var batchId = 'stage42e-' + Date.now() + '-' + Math.random().toString(36).slice(2);
      var rawBatch = imported;
      ...
      if (importDeck.aiRepairPending) {
        ...
        try{ globalThis.__LUMINA_STAGE42E_BACKGROUND_AI_REPAIR = { ok:null, pending:true, applied:false, batchId:batchId, startIndex:startIndex, slideCount:rawBatch.length, at:new Date().toISOString() }; }catch(_err){}
  ...
  try {
    ...
    global.__LUMINA_STAGE41V_FILE_IO_READY = { stage:'stage42l-import-flow-restore-ai-stale-fix-20260511-1', ready:true, at:new Date().toISOString(), apiKeys:Object.keys(api) };
