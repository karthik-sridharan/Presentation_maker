...
.slide-actions{position:absolute;top:1.2rem;right:1.2rem;display:flex;gap:.55rem;z-index:10;flex-wrap:wrap;justify-content:flex-end;align-items:center;max-width:min(720px,calc(100% - 2.4rem))}
...
.laser-pointer[data-pointer-color="black"]{--laser-rgb:0,0,0;--laser-border-rgb:0,0,0}
...
deck.innerHTML=deckPayload.slides.map((slide,idx)=>{const cls=slide.slideType==='title-center'?'deck-slide title-center':(slide.slideType==='two-col'?'deck-slide two-col':'deck-slide single');const styleCls=' style-'+String((deckPayload.theme&&deckPayload.theme.beamerStyle)||'classic').replace(/[^a-z0-9_-]/gi,'').toLowerCase();const laserControl='<label class="laser-control">Pointer <select class="laser-select" aria-label="Laser pointer color"><option value="red" selected>Red</option><option value="blue">Blue</option><option value="green">Green</option><option value="black">Black</option><option value="none">None</option></select></label>';const actionHtml='<div class="slide-actions"><button class="slides-button" type="button">Slides</button>'+(liveDrawEnabled?'<button class="draw-button" type="button">Draw</button><button class="export-annotated-button" type="button">Export annotated slides</button>':'')+laserControl+'<button class="pdf-button" type="button">Generate PDF</button></div>';return '<section class="'+cls+styleCls+'" data-index="'+idx+'" style="'+buildSlideStyle(slide)+'">'+actionHtml+'<div class="slide-number">'+(idx+1)+' / '+deckPayload.slides.length+'</div>'+buildSlideInner(slide).trim()+'<div class="slide-annotation-layer" data-annotation-layer="'+idx+'"><svg class="slide-draw-surface" data-draw-surface="'+idx+'" viewBox="0 0 1000 640" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" aria-label="Slide annotation layer"></svg></div></section>';}).join('\\n');
...
function setLaserPointerMode(mode){mode=['red','blue','green','black','none'].includes(mode)?mode:'red';laserPointerMode=mode;if(laserPointer){laserPointer.dataset.pointerColor=mode;if(mode==='none') hideLaserPointer();}laserSelects.forEach(sel=>{if(sel.value!==mode) sel.value=mode;});}
