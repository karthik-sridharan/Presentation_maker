/* Stage 27C: non-blocking ES module smoke/parity checks.
   Hotfix from 27A: use .js module assets instead of .mjs so common static hosts
   serve the files with a browser-accepted JavaScript MIME type.
...
import * as utils from './esm/utils-stage27c.js';
import { createApi as createBlockStyleApi } from './esm/block-style-stage27c.js';

const W = window;
const STAGE = 'stage27c-20260425-1';
const checks = [];
...
markLoaded('js/esm/utils-stage27c.js');
markLoaded('js/esm/block-style-stage27c.js');
...
markLoaded('js/es-module-smoke-stage27c.js');

if(!report.ok){
  if(W.LuminaDiagnostics && typeof W.LuminaDiagnostics.warn === 'function'){
    W.LuminaDiagnostics.warn('Stage 27C ES module smoke checks failed: ' + failed.map((item)=>item.name).join(', '));
  }
}
