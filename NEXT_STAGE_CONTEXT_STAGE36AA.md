# Context for Continuing After Stage 36AA
...
- `js/esm/copilot-stage34k.js` — guarded ESM Copilot core, patched in Stage 36AA.
...
## Stage 36AA note
The selected item inspector now stores actual pixel font sizes as `style.fontSize` (for example `24px`). Existing `style.fontScale` fields remain supported as a fallback so old decks still render. The renderer/export CSS prefers `--block-font-size` over scaled default sizes when present.
